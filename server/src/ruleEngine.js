...
//    クリリン(17, 表示文字列に59を含む) > ネコ(15) > 班長(23, 11/111/1111) >
//    名言(16, 42) > ナベアツ(14) > 3歳児(20, 100以上) の順。
//    （例：222はナベアツの条件も満たすがネコが優先。593は59を含みナベアツの
//    条件(3を含む)も満たすがクリリンが優先。42はナベアツの条件も満たすが名言が優先。
//    111はナベアツの条件も満たすが班長が優先。111・1111は3歳児の条件(100以上)も
//    満たすが班長が優先。）
...
// 10. v2で変更: ルール17「クリリン」は当初「結果が59または593と完全一致」だったが、
//     59は素数で結果として出にくいため、「結果の文字列表現に部分文字列として
//     "59"を含むか」に一般化した(ルール14のアホの「3を含む」判定と同じ考え方)。
//     591〜599、1591〜1599、-59、0.59なども対象になる。593も"59"を含むため、
//     この一般化に自然に吸収される。
...
// ルール17「クリリン」の発動条件: 計算結果の表示文字列(formatNumber基準)に
// 部分文字列として"59"を含むか。59は素数で完全一致だと結果として出にくいため、
// 591〜599やマイナス・小数を含む「59を含む数字全般」に一般化している。
function containsFiftyNine(finalValue) {
  return formatNumber(finalValue).includes('59');
}

function applyStageD(finalValue, ruleIds, trace) {
  const has = (id) => ruleIds.includes(id);

  if (has(17) && containsFiftyNine(finalValue)) {
